...
const mergeCvs = (cvs) => {
  ...
  const lastCvLastV = cvs.reverse()[0].split(":")[1].split("-").reverse()[0];
...
const usage = "node ultReglossJuxta.js <jxlPath> <bsbTsvPath>";
...
for (let [sentenceN, sentence] of jxlJson.entries()) {
  let chunkEntries = sentence.chunks.entries();
  for (const [chunkN, chunk] of chunkEntries) {
    // noun <=> adjective
    if (
      chunk.source.length == 2 &&
      chunk.source[0].morph[1] == "N" &&
      chunk.source[1].morph[1] == "AA"
    ) {
...
for (let [sentenceN, sentence] of jxlJson.entries()) {
  for (const chunk of sentence.chunks) {
    ...
    for (const [sourceN, source] of chunkSourceEntries) {
      if (
        sourceN > 0 &&
        chunkSourceEntries[sourceN - 1][1].morph[1] === "N" &&
        source.morph[1] === "RP" &&
        source.morph[4] === "1G" &&
        source.morph[5] === "S"
      ) {
      ...
      chunk.gloss = chunk.gloss.replace(/-+/g, "-");
    }
  }
}
